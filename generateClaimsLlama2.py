...
def cache_claims_prompts():
    ...
    df = pd.DataFrame(columns=['i', 'claims_prompt'])
    ...
    print("writing to file", constants.CACHED_CLAIMS_PROMPTS_FILES)
    df.to_csv(constants.CACHED_CLAIMS_PROMPTS_FILES, index=False)

# cannot run the sentenceTransformer needed for RAG at the same time as llama2, so we can only do one at a time.
# uncomment to cache the claims prompts
# cache_claims_prompts()
# exit()
...
processed_patents_df = pd.read_parquet(constants.PROCESSED_FILE)
generated_abstracts_df = pd.read_csv(constants.GENERATED_ABSTRACTS_FILE)
cached_claims_prompts_df = pd.read_csv(constants.CACHED_CLAIMS_PROMPTS_FILES)
...
for row_num, patent in generated_abstracts_df.iterrows():
    print("--------------------------------------------------")
    ...
    claims_prompt = cached_claims_prompts_df.iloc[row_num]["claims_prompt"]
    print('len claims_prompt: ', len(claims_prompt))

    generated_US_claims = llama.generate(US_claims_system, claims_prompt, max_tokens_value=max_tokens_value)
